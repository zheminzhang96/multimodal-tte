...
FEATURE_COLUMNS = {
    "CLMBR": "features",
    "ONEHOT": "1hot_features",
}


def get_folder_name(slice_type, window):
    if slice_type not in {"LCS_slice", "heart"}:
        raise ValueError("slice_type must be 'LCS_slice' or 'heart'")
    if window not in {"full", "soft"}:
        raise ValueError("window must be 'full' or 'soft'")
    if window == "full":
        return slice_type
    return f"{slice_type}_soft"


def build_embedding_path(embedding_root, slice_type, window, split, rec):
    if split not in VALID_SPLITS:
        raise ValueError(f"split must be one of {sorted(VALID_SPLITS)}")

    uid = f"{rec['patient_id']}_{rec['accession_number']}"
    root = Path(embedding_root) / get_folder_name(slice_type, window) / split
    if slice_type == "LCS_slice":
        idx = int(float(rec["largest_slice"]))
        return root / f"{uid}_{idx}_embedding.npy"
    return root / f"{uid}_embeddings.npy"
...
def iter_embeddings(
    ...
    for _, row in df.iterrows():
        ...
        rec = {
            ...
            "largest_slice": int(row["heart_largest_cross_section_slice_index"]),
            "heart_high_index": int(row["heart_highest_slice_index"]),
            "heart_low_index": int(row["heart_lowest_slice_index"]),
            "time": float(row["MACE_or_DEATH_month"]),
            "event": int(row["MACE_or_DEATH_event"]),
            "path": str(row.get("NIFTI location-Axial-v2", "")),
            "features": features,
        }

        emb_path = build_embedding_path(embedding_root, slice_type, window, split, rec)
...
class ASCVDMIIDataset(Dataset):
    def __init__(self, df, embedding_root, split, tabular_col, slice_type="heart", window="soft"):
        self.samples = []
        for rec, arr_mean in iter_embeddings(
            df, embedding_root, slice_type, window, split, tabular_col
        ):
...
class ASCVDMIIDatasetNoise(ASCVDMIIDataset):
    def __init__(
        self,
        df,
        embedding_root,
...
def build_datasets(
    ...
    for split in ("train", "val", "test"):
        split_df = df[df["split_name"] == split]
        datasets[split] = ASCVDMIIDataset(
            split_df, embedding_root, split, tabular_col, slice_type=slice_type, window=window
        )

    datasets["test_noise"] = ASCVDMIIDatasetNoise(
        df[df["split_name"] == "test"],
        embedding_root,
        "test",
        tabular_col,
    ...
    if external_metadata_file:
        df_external = read_metadata(external_metadata_file)
        external_root = external_embedding_root or embedding_root
        datasets["external"] = ASCVDMIIDataset(
            df_external,
            external_root,
            "external",
            tabular_col,
        ...
        datasets["external_noise"] = ASCVDMIIDatasetNoise(
            df_external,
            external_root,
            "external",
            tabular_col,
